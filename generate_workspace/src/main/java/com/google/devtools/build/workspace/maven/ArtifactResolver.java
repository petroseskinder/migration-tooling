...
public class ArtifactResolver {
   ...
   */
  public DependencyNode resolveArtifacts(List<String> artifacts) {
    DependencyRequest dependencyRequest = createDependencyRequest(artifacts);
   ...
   */
  private DependencyRequest createDependencyRequest(List<String> artifacts) {

    Function<String, Dependency> coordinateToDependencyNode =
        a -> new Dependency(new DefaultArtifact(a), JavaScopes.COMPILE);

    List<Dependency> roots = artifacts.stream().map(coordinateToDependencyNode).collect(toList());
